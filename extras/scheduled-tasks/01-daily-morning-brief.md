# Scheduled Task · Morning Brief · Daily · Suggested 08:10
...
1. Yesterday's scorecard: yesterday's net GMV, number of videos that drove orders, number of new creators producing video, compared with the day before (query_collaboration_performance / query_shoppable_video_performance).
2. Health of running automations: list_automations to see which are active; flag any zero-output automations (0 accepts / 0 videos) for human inspection.
3. Pending replies: list_conversations for unhandled reply count (count + rough classification only, do not reply).
4. Today's Top 5: combining yesterday's winners, creators near the graduation line, and post-sample silence — give the 5 most important things to do today (one line each + reason + matching prompt number).
5. Risk radar: take-rate > 35%, suspected duplicate outreach, suspected compliance-word hits — list them, or write "none".
Persist to local file `morning-brief_YYYY-MM-DD.md`; paste a ≤6-line summary in the conversation.
```
