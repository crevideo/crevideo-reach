...
> 1 个基础 skill + 8 个业务场景 skill + Crevideo Reach MCP（89 个工具）+ 5 个定时任务模板 + 2 个 Project Instructions
...
2. **MCP 工具自动连接** —— 首次调用工具时弹浏览器走 OAuth → 跳到 app.crevideo.com 登录你的 Reach 账号 → 回来后 89 个工具都可以用
...
### 🟢 MCP Server: `crevideo-reach`（89 个工具）
...
- **DM 私信**：会话 / 历史 / 发送文本、图片、图文卡片和商品卡片
- **邮件**：模板 / 草稿 / 收件箱 / 发件 / 回复 / 分组 / 回收站

多国家使用规则：先调用 `list_shops` 获取账户真实授权市场，再保持 `region`、`shop_id_list`、`shop_cipher` 作用域一致。GMV/GPM 筛选使用各市场当地币种的档位和展示规范，不能套用美国范围。
- **分销内容**：列表 / 商品
