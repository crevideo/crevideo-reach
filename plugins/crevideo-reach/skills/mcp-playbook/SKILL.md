...
- **DM conversations:** `list_conversations`, `get_conversation_messages`, `send_message` (real outbound — text OR image_path; confirm), `search_conversations` (find an EXISTING DM by @handle; **returns nothing ⇒ no thread yet ⇒ go to create_conversation, do NOT give up**), `create_conversation` (open a NEW conversation, idempotent), `list_conversation_groups` (left-rail groups + counts), `list_group_conversations` (conversations in one group — use for "show unread DMs"), `manage_conversation_groups` (create/rename/delete a folder, move/remove a conversation — confirm).
...
- 🚨 **Can't find a creator's conversation? (not in `list_conversations`, `search_conversations` returns nothing, or you only have their @handle):** this means NO DM thread exists yet — it does **NOT** mean they're unreachable. **Do NOT stop or report "can't message them".** Open a thread first, then send:
  `search_affiliate_creators(creator_keyword='@handle')` → `user_id` → `create_conversation(creator_user_ids=[user_id])` → `send_message`. This path is **FREE** (no MCP credits) and is the exact equivalent of the brand-app's "open a chat from the creator's profile". This is the normal way to DM creators you haven't talked to yet (e.g. a list of creators whose videos you want to follow up on).
