...
## Sample approval (申样)
`manage_sample_auto_approval` `get`/`update` sets the rule thresholds; `manage_sample_applications` `list` uses `region` + optional decimal shop IDs in `shop_id_list` (preserve the exact ID string; do not round a 19-digit ID through a JavaScript number; omit for all authorized shops in that region; never pass `shop_cipher`) → `approve` / `reject` uses `shop_cipher` + `application_ids` + `confirm:true` to clear what the rule did not cover. For per-TC cleanup evidence, use `get_target_collaboration_detail(include_sample_audit=true)` instead of inferring from the generic sample queue.
