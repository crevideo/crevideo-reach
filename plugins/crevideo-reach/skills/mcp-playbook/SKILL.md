...
- **DM conversations:** `list_conversations`, `get_conversation_messages`, `send_message` (real outbound — text OR image_path; confirm), `search_conversations` (find one creator's DM by @handle — instead of paging), `create_conversation` (open a NEW conversation, idempotent), `list_conversation_groups` (left-rail groups + counts), `list_group_conversations` (conversations in one group — use for "show unread DMs"), `manage_conversation_groups` (create/rename/delete a folder, move/remove a conversation — confirm).
