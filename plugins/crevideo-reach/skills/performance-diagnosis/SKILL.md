...
2. **Field traps**: `*_increment` is the **total for the selected period**, not an increment / growth rate — do not read it as a growth rate; breakdown returns the full set, take the first rows as "top N". Write N/A for any number you can't get and say which query failed — **never fabricate**.
